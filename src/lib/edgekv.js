...
export class EdgeKV {
  ...
  constructor(namespace = "default", group = "default") {
    if (typeof namespace === "object") {
      ...
      this.#edgekv_uri =
        namespace.edgekv_uri || "https://edgekv.akamai-edge-svcs.net";
      this.#token_override = namespace.token_override || null;
      this.#num_retries_on_timeout = namespace.num_retries_on_timeout || 0;
      this.#sandbox_id = namespace.ew_request
        ? namespace.ew_request.sandboxId || null
        : null;
  ...
  async requestHandlerTemplate(
    ...
    try {
      ...
      if (
        ...
        /^.*subrequest to URL.*timed out.*$/.test(error.toString())
      ) {
  ...
  getNamespaceTokenHeader(namespace) {
    ...
    if (!(name in edgekv_access_tokens)) {
      throw (
        "MISSING ACCESS TOKEN. No EdgeKV Access Token defined for namespace '" +
        namespace +
        "'."
      );
    }
    if ("value" in edgekv_access_tokens[name]) {
      ...
      return {
        "X-Akamai-EdgeDB-Auth-Ref": [edgekv_access_tokens[name]["reference"]],
      };
    } else {
      throw (
        "MISSING ACCESS TOKEN. No EdgeKV Access Token value or reference defined for namespace '" +
        namespace +
        "'."
      );
    }
  ...
  async streamText(response_body) {
    let result = "";
    await response_body.pipeThrough(new TextDecoderStream()).pipeTo(
      new WritableStream({
        write(chunk) {
          result += chunk;
        },
